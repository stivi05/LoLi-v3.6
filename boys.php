...
global $CacheBlock, $rootpath;$_cache = 'boys.cache';$boys_cache = $rootpath."include/cache/boys.cache";if(!file_exists($boys_cache)){
$res1 = sql_query("SELECT COUNT(id) FROM users WHERE gender = '1'");$row1 = mysql_fetch_array($res1);$count = $row1[0];
...
$query_string = "SELECT users.id, users.username, users.avatar, users.added, users.country, users.downloaded, users.uploaded, users.class, users.birthday, users.last_access, c.name, c.flagpic FROM users LEFT JOIN countries AS c ON c.id = users.country WHERE users.gender = '1' $limit";
...
<a href='#' onclick=\"javascript:window.open('sendpm_".$arr['id']."', 'Отправить PM', 'width=650px, height=465px');return false;\" title='Отправить ЛС'><img src='pic/pn_inbox.gif' border='0' title='PM'/></a>&nbsp;&nbsp;$country<br><br><img src='pic/multitracker.png' border='0' alt='Рейтинг'/>&nbsp;<b>$ratio</b></center></td></tr></table><hr width='200'>
<table style='background:none;border:0;' width='240px'><tr><td class='embedded' align='left' style='background:none;border:0;'>
<b>Дата Рождения:</b>&nbsp;<font color='darkred'><b>".$arr['birthday']."&nbsp;(".$age." лет)</b></font></td></tr><tr>
<td class='embedded' align='left' style='background:none;border:0;'>
