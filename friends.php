...
if(!is_valid_id($userid)) stderr($tracker_lang['error'], "<center>".$tracker_lang['invalid_id']."</center><html><head><meta http-equiv=refresh content='3;url=/'></head></html>");
if($userid != $CURUSER["id"])stderr($tracker_lang['error'], "<center>".$tracker_lang['access_denied']."</center><html><head><meta http-equiv=refresh content='3;url=/'></head></html>");
if($action == 'add'){$targetid = intval($_GET['targetid']);$type = strval($_GET['type']);
if(!is_valid_id($targetid))stderr($tracker_lang['error'], "<center>".$tracker_lang['invalid_id']."</center><html><head><meta http-equiv=refresh content='3;url=/'></head></html>");
if($type == 'friend'){$table_is = $frag = 'friends';$field_is = 'friendid';}elseif($type == 'block'){$table_is = $frag = 'blocks';$field_is = 'blockid';}
else stderr($tracker_lang['error'], "<center>Unknown type.</center><html><head><meta http-equiv=refresh content='3;url=/'></head></html>");
$r = mysql_query("SELECT id FROM $table_is WHERE userid=$userid AND $field_is=$targetid") or sqlerr(__FILE__, __LINE__);
if(mysql_num_rows($r) == 1)stderr($tracker_lang['error'], "<center>User ID is already in your ".htmlentities($table_is)." list.</center><html><head><meta http-equiv=refresh content='3;url=/'></head></html>");
mysql_query("INSERT INTO $table_is VALUES (0, $userid, $targetid)") or sqlerr(__FILE__, __LINE__);header("Location: friends");die;}
...
if(mysql_affected_rows() == 0)stderr($tracker_lang['error'], "<center>".$tracker_lang['invalid_id']."</center><html><head><meta http-equiv=refresh content='3;url=/'></head></html>");$frag = "friends";}
elseif($type == 'block'){
mysql_query("DELETE FROM blocks WHERE userid=$userid AND blockid=$targetid") or sqlerr(__FILE__, __LINE__);
if(mysql_affected_rows() == 0)stderr($tracker_lang['error'], "<center>".$tracker_lang['invalid_id']."</center><html><head><meta http-equiv=refresh content='3;url=/'></head></html>");$frag = "blocks";
}else stderr($tracker_lang['error'], "Unknown type.");header("Location: friends");die;}
//////////////////////////////
stdhead("Мои Дузья / Враги");?><table style="background:none;cellspacing:0;cellpadding:0;width:100%;float:center;"><tr>
...
.:: <?=$tracker_lang['friends_list']?> ::.</td></tr><tr><td align="center" style="background:none;width:100%;float:center;border:0;"></td></tr></table></td></tr></table><?
...
<td style='border-radius:5px;margin-top:7px;-webkit-border-radius:5px;-moz-border-radius:5px;-khtml-border-radius:5px;border:1px solid white;display:block;' class='a'>
...
elseif($friend["gender"] == "3") $gender = "<img src='pic/transgenders.gif' alt='Трансгендер' style='margin-left:4pt;'/>";
if(!$friend['avatar']){$avatar = "<a href='userdetails.php?id=".$friend['id']."'><img style='border-radius:5px;border:0;height:80px;' title='".$friend['username']."' src='pic/default_avatar.gif'/></a>";
}else{$avatar = "<a href='userdetails.php?id=".$friend['id']."'><img style='border-radius:5px;border:0;height:80px;' src='".$friend['avatar']."' title='".$friend['username']."'/></a>";}
print("<td style='padding:7px;width:240px;background:none;'><table style='background:none;border:0;' cellspacing='0'><tr>
...
<table style='background:none;align:center;border:0;' width='240px'><tr><td class='embedded' width='90px' style='background:none;align:center;border:0;'><center>$avatar</center>
</td><td class='embedded' valign='top' align='center' style='background:none;border:0;'><center>
<a class='altlink' href='userdetails.php?id=".$friend['id']."'><b>".get_user_class_color($friend['class'],$friend['username'])."</b></a>".($friend["donated"] > 0 ? "
<img src='pic/star.gif' border='0' alt='Donor'/>" : "")."&nbsp;&nbsp;$gender
<a href='friends.php?id=$userid&action=delete&type=friend&targetid=".$friend['id']."'><img align='right' title='".$tracker_lang['delete']." из Друзей' alt='".$tracker_lang['delete']." из Друзей' src='pic/close.png'/></a><hr width='100px'>
...
<a href='#' onclick=\"javascript:window.open('sendpm.php?action=sendmessage&receiver=".$friend['id']."', 'Отправить PM', 'width=650px, height=465px');return false;\" title='Отправить ЛС'><img src='pic/pn_inbox.gif' border='0' title='PM'/></a>&nbsp;&nbsp;$country<br/><br/><img src='pic/multitracker.png' border='0' alt='Рейтинг'/>&nbsp;<b>$ratio</b></center>
</td></tr></table><hr width='200px'><table style='background:none;border:0;' width='240px'><tr><td class='embedded' align='left' style='background:none;border:0;'>
...
<td style="border-radius:15px;border:none;" class='a'><table style="background:none;width:100%;float:center;border:0;"><tr>
<td class="zaliwka" style="color:#FFFFFF;colspan:14;height:20px;font-family:cursive;font-weight:bold;font-size:14px;text-align:center;border:0;border-radius:5px;">
.:: <?=$tracker_lang['blocked_list']?> ::.</td></tr><tr><td align="center" style="background:none;width:100%;float:center;border:0;"></td></tr></table></td></tr></table><?
...
elseif($block["gender"] == "3") $gender = "<img src='pic/transgenders.gif' alt='Трансгендер' style='margin-left:4pt;'/>";
if(!$block['avatar']){$avatar = "<a href='userdetails.php?id=".$block['id']."'><img style='border-radius:5px;border:0;height:80px;' title='".$block['username']."' src='pic/default_avatar.gif'/></a>";
}else{$avatar = "<a href='userdetails.php?id=".$block['id']."'><img style='border-radius:5px;border:0;height:80px;' src='".$block['avatar']."' title='".$block['username']."'/></a>";}
print("<td style='padding:7px;width:240px;background:none;'><table style='background:none;border:0;' cellspacing='0'><tr>
<td style='border-radius:10px;-webkit-border-radius:8px;-moz-border-radius:8px;-khtml-border-radius:8px;border:2px solid #E0FFFF;display:block;padding:7px;width:240px;' class='a'>
<table style='background:none;align:center;border:0;' width='240px'><tr><td class='embedded' width='90px' style='background:none;align:center;border:0;'><center>$avatar</center>
</td><td class='embedded' valign='top' align='center' style='background:none;border:0;'><center>
<a class='altlink' href='userdetails.php?id=".$block['id']."'><b>".get_user_class_color($block['class'],$block['username'])."</b></a>".($block["donated"] > 0 ? "
<img src='pic/star.gif' border='0' alt='Donor'/>" : "")."&nbsp;&nbsp;".$gender."
<a href='friends.php?id=$userid&action=delete&type=block&targetid=".$block['id']."'><img align='right' title='".$tracker_lang['delete']." из Врагов' alt='".$tracker_lang['delete']." из Врагов' src='pic/close.png'/></a><hr width='100px'>
...
<a href='#' onclick=\"javascript:window.open('sendpm.php?action=sendmessage&receiver=".$block['id']."', 'Отправить PM', 'width=650px, height=465px');return false;\" title='Отправить ЛС'><img src='pic/pn_inbox.gif' border='0' title='PM'/></a>&nbsp;&nbsp;$country<br/><br/><img src='pic/multitracker.png' border='0' alt='Рейтинг'/>&nbsp;<b>$ratio</b></center>
</td></tr></table><hr width='200px'><table style='background:none;border:0;' width='240px'><tr><td class='embedded' align='left' style='background:none;border:0;'>
...
++$ncs;if($ncs == 4){$ncs=1;print("</tr>");}}}print("</table>");}stdfoot();}else{?><html><head><meta http-equiv='refresh' content='0;url=/'></head>
