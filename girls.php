...
list($pagertop, $pagerbottom, $limit) = pager2($limit, $count, "girls.php?", array('nopage' => true));
...
.:: Девушки нашего трекера <img src='pic/female0.gif'/> :: посмотреть <a class='but' href='boys.php'>Парни нашего трекера</a> :: или вы хотите посмотреть <a class='but' href='transgender.php'>Трансиков нашего трекера?</a> ::.</td></tr></table></td></tr></table><?
$dt = gmtime() - 300;$dt = sqlesc(get_date_time($dt));
$query_string = "SELECT users.id, users.username, users.avatar, users.added, users.country, users.downloaded, users.uploaded, users.class, users.birthday, users.last_access, c.name, c.flagpic FROM users LEFT JOIN countries AS c ON c.id = users.country WHERE users.gender = '2' $limit";
...
elseif($arr["uploaded"] > 0) $ratio = "Inf.";else $ratio = "Inf.";$gender = "<img src='pic/female1.gif' alt='Девушка' style='margin-left:4pt;'/>";
if(!$arr[avatar]){$avatar=("<a href='users_".$arr['id']."'><img border='0' height='80px' title='".$arr['username']."' src='pic/default_avatar.gif'/></a>");
}else{$avatar=("<a href='users_".$arr['id']."'><img height='80px' src='".$arr['avatar']."' title='".$arr['username']."'/></a>");}
print("<td style='padding:10px;width:240px;background:none;'><table style='background:none;'border='0' cellspacing='0'><tr>
<td style='border-radius:10px;-webkit-border-radius:8px;-moz-border-radius:8px;-khtml-border-radius:8px;border:2px solid #E0FFFF;display:block;padding:10px;width:240px;' class='a'>
...
echo "<table style='background:none;border:none;cellspacing:0;cellpadding:0;margin-top:7px;width:200px;float:center;'><tr><td style='margin-top:7px;width:250px;float:center;border-radius:5px;-webkit-border-radius:5px;-moz-border-radius:5px;-khtml-border-radius:5px;border:1px solid white;display:block;' class='a'><center><font style='font-family:tahoma;font-size:14px;font-weight:10;color:red;'><b>НЕТ Девушек на сайте, кругом одни пацаны! Но гляньте еще на Трансиков! Они везде! Возможно они и у нас есть! Интересно? Сверху ссылка!</b></font></center></td></tr></table>";	
