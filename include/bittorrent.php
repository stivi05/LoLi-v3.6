...
@error_reporting(E_ALL & ~E_NOTICE);@ini_set('error_reporting', E_ALL & ~E_NOTICE);@ini_set('display_errors', '1');@ini_set('display_startup_errors', '0');
@ini_set('ignore_repeated_errors', '1');@ignore_user_abort(1);@set_time_limit(0);@session_start();
...
define ('TIMENOW', time());
$url = explode('/', detect_sqlinjection($_SERVER['PHP_SELF']));array_pop($url);
$DEFAULTBASEURL = (($_SERVER['SERVER_PORT'] == 443) ? "https://" : "https://").htmlspecialchars_uni($_SERVER['HTTP_HOST']).implode('/', $url);
$BASEURL = $DEFAULTBASEURL;$announce_urls = array();$announce_urls[] = "$DEFAULTBASEURL/announce.php";
// SECURITY
...
define('LoLi', 'Powered by LoLi v4.0 Copyright &copy; 2017-'.date('Y'));
//////////////////////////
...
$use_email_act = 1; // 1 - Использовать активацию по почте, 0 - автоматическая активация при регистрации.
$use_lang = 1; // 1 - Включить языковую систему. 0 - Выключите если вы хотите перевести шаблоны и другие файлы - тогда все фразы от системы станут пустым местом.
...
function RowsTextarea(n, w){var inrows = document.getElementById(n);if (w < 1){var rows = -5;}else{var rows = +5;}var outrows = inrows.rows + rows;
if(outrows >= 5 && outrows < 50){inrows.rows = outrows;}return false;}
var SelField = document.<?php echo $form;?>.<?php echo $name;?>;var TxtFeld  = document.<?php echo $form;?>.<?php echo $name;?>;
var clientPC = navigator.userAgent.toLowerCase();var clientVer = parseInt(navigator.appVersion);
var is_ie = ((clientPC.indexOf("msie") != -1) && (clientPC.indexOf("opera") == -1));
var is_nav = ((clientPC.indexOf('mozilla')!=-1) && (clientPC.indexOf('spoofer')==-1)&& (clientPC.indexOf('compatible') == -1) && (clientPC.indexOf('opera')==-1) 
&& (clientPC.indexOf('webtv')==-1) && (clientPC.indexOf('hotjava')==-1));
var is_moz = 0;var is_win = ((clientPC.indexOf("win")!=-1) || (clientPC.indexOf("16bit") != -1));var is_mac = (clientPC.indexOf("mac")!=-1);
function StoreCaret(text){if(text.createTextRange){text.caretPos = document.selection.createRange().duplicate();}}
function FieldName(text, which){if(text.createTextRange){text.caretPos = document.selection.createRange().duplicate();}
if(which != ""){var Field = eval("document.<?php echo $form;?>."+which);SelField = Field;TxtFeld  = Field;}}
function AddSmile(SmileCode){var SmileCode;var newPost;var oldPost = SelField.value;newPost = oldPost+SmileCode;SelField.value=newPost;SelField.focus();return;}
function AddSelectedText(Open, Close){if(SelField.createTextRange && SelField.caretPos && Close == '\n'){var caretPos = SelField.caretPos;
caretPos.text = caretPos.text.charAt(caretPos.text.length - 1) == ' ' ? Open + Close + ' ' : Open + Close;SelField.focus();
}else if(SelField.caretPos){SelField.caretPos.text = Open + SelField.caretPos.text + Close;}else{SelField.value += Open + Close;SelField.focus();}}
function InsertCode(code, info, type, error){if(code == 'name'){AddSelectedText('[b]' + info + '[/b]', '\n');
}else if(code == 'url' || code == 'mail'){if(code == 'url') var url = prompt(info, 'http://');if(code == 'mail') var url = prompt(info, '');if(!url) return alert(error);
if((clientVer >= 4) && is_ie && is_win){selection = document.selection.createRange().text;
if(!selection){var title = prompt(type, type);AddSelectedText('[' + code + '=' + url + ']' + title + '[/' + code + ']', '\n');}else{
AddSelectedText('[' + code + '=' + url + ']', '[/' + code + ']');}}else{mozWrap(TxtFeld, '[' + code + '=' + url + ']', '[/' + code + ']');}
}else if(code == 'color' || code == 'family' || code == 'size'){if((clientVer >= 4) && is_ie && is_win){AddSelectedText('[' + code + '=' + info + ']', '[/' + code + ']');
}else if(TxtFeld.selectionEnd && (TxtFeld.selectionEnd - TxtFeld.selectionStart > 0)){mozWrap(TxtFeld, '[' + code + '=' + info + ']', '[/' + code + ']');}
}else if(code == 'li' || code == 'hr'){if((clientVer >= 4) && is_ie && is_win){AddSelectedText('[' + code + ']', '');}else{mozWrap(TxtFeld, '[' + code + ']', '');}
}else if(code == 'spoiler'){mozWrap(TxtFeld, '[' + code + '=Скрытая информация]', '[/' + code + ']');}else{
if((clientVer >= 4) && is_ie && is_win){var selection = false;selection = document.selection.createRange().text;
if(selection && code == 'quote'){AddSelectedText('[' + code + ']' + selection + '[/' + code + ']', '\n');}else{AddSelectedText('[' + code + ']', '[/' + code + ']');}}
else{mozWrap(TxtFeld, '[' + code + ']', '[/' + code + ']');}}}
function mozWrap(txtarea, open, close){var selLength = txtarea.textLength;var selStart = txtarea.selectionStart;var selEnd = txtarea.selectionEnd;
if(selEnd == 1 || selEnd == 2)selEnd = selLength;var s1 = (txtarea.value).substring(0,selStart);var s2 = (txtarea.value).substring(selStart, selEnd)
var s3 = (txtarea.value).substring(selEnd, selLength);var sT = txtarea.scrollTop, sL = txtarea.scrollLeft;txtarea.value = s1 + open + s2 + close + s3;
txtarea.focus();txtarea.scrollTop=sT;xtarea.scrollLeft=sL;return;}
language=1;richtung=1;var DOM = document.getElementById ? 1 : 0, opera = window.opera && DOM ? 1 : 0, IE = !opera && document.all ? 1 : 0, NN6 = DOM && !IE && !opera ? 1 : 0; 
var ablauf = new Date();var jahr = ablauf.getTime() + (365 * 24 * 60 * 60 * 1000);ablauf.setTime(jahr);var richtung=1;var isChat=false;NoHtml=true;NoScript=true;
NoStyle=true;NoBBCode=true;NoBefehl=false;
function setZustand(){transHtmlPause=false;transScriptPause=false;transStylePause=false;transBefehlPause=false;transBBPause=false;}setZustand();
function keks(Name,Wert){document.cookie = Name+"="+Wert+"; expires=" + ablauf.toGMTString();}
function changeNoTranslit(Nr){if(document.trans.No_translit_HTML.checked)NoHtml=true;else{NoHtml=false}
if(document.trans.No_translit_BBCode.checked)NoBBCode=true;else{NoBBCode=false}keks("NoHtml",NoHtml);keks("NoScript",NoScript);keks("NoStyle",NoStyle);keks("NoBBCode",NoBBCode);}
function changeRichtung(r){richtung=r;keks("TransRichtung",richtung);setFocus()}
function changelanguage(){if(language==1){language=0;}else{language=1;}keks("autoTrans",language);setFocus();setZustand();}function setFocus(){TxtFeld.focus();}
function repl(t,a,b){var w=t,i=0,n=0;while((i=w.indexOf(a,n))>=0){
t=t.substring(0,i)+b+t.substring(i+a.length,t.length);w=w.substring(0,i)+b+w.substring(i+a.length,w.length);n=i+b.length;if(n>=w.length){break;}}return t;}
...
var transAN=true;
function transliteText(txt){vorTxt=txt.length>1?txt.substr(txt.length-2,1):"";buchstabe=txt.substr(txt.length-1,1);txt=txt.substr(0,txt.length-2);return txt+translitBuchstabeCyr(vorTxt,buchstabe);}
function translitBuchstabeCyr(vorTxt,txt){var zweiBuchstaben = vorTxt+txt;var code = txt.charCodeAt(0);if(txt=="<")transHtmlPause=true;else if(txt==">")transHtmlPause=false;
if(txt=="<script")transScriptPause=true;else if(txt=="<"+"/script>")transScriptPause=false;if(txt=="<style")transStylePause=true;else if(txt=="<"+"/style>")transStylePause=false;
if(txt=="[")transBBPause=true;else if(txt=="]")transBBPause=false;if(txt=="/")transBefehlPause=true;else if(txt==" ")transBefehlPause=false;
if((transHtmlPause==true &&   NoHtml==true)||(transScriptPause==true &&   NoScript==true)||(transStylePause==true &&   NoStyle==true)||(transBBPause==true &&   NoBBCode==true)||
(transBefehlPause==true &&   NoBefehl==true)||!(((code>=65) && (code<=123))||(code==35)||(code==39))) return zweiBuchstaben;
for(x=0; x<lat_lr2.length; x++){if (lat_lr2[x]==zweiBuchstaben) return rus_lr2[x];}for(x=0; x<lat_lr1.length; x++){if (lat_lr1[x]==txt) return vorTxt+rus_lr1[x];}return zweiBuchstaben;}
function translitBuchstabeLat(buchstabe){for (x=0; x<rus_rl.length; x++){if (rus_rl[x]==buchstabe)return lat_rl[x];}return buchstabe;}
function translateAlltoLatin(){if(!IE){var txt=TxtFeld.value;var txtnew = "";var symb = "";for (y=0;y<txt.length;y++){
symb = translitBuchstabeLat(txt.substr(y,1));txtnew += symb;}TxtFeld.value = txtnew;setFocus()}else{
var is_selection_flag = 1;var userselection = document.selection.createRange();var txt = userselection.text;
if(userselection==null || userselection.text==null || userselection.parentElement==null || userselection.parentElement().type!="textarea"){is_selection_flag = 0;txt = TxtFeld.value;}
txtnew="";var symb = "";for (y=0;y<txt.length;y++){symb = translitBuchstabeLat(txt.substr(y,1));txtnew +=  symb;}
if(is_selection_flag){userselection.text = txtnew; userselection.collapse(); userselection.select();}else{TxtFeld.value = txtnew;setFocus()}}return;}
function TransliteFeld(object, evnt){
if(language==1 || opera) return;if(NN6){var code=void 0;var code =  evnt.charCode;var textareafontsize = 14;var textreafontwidth = 7;if(code == 13){return;}
if( code && (!(evnt.ctrlKey || evnt.altKey))){pXpix = object.scrollTop;pYpix = object.scrollLeft;evnt.preventDefault();txt=String.fromCharCode(code);
pretxt = object.value.substring(0, object.selectionStart);result = transliteText(pretxt+txt);object.value = result+object.value.substring(object.selectionEnd);
object.setSelectionRange(result.length,result.length);object.scrollTop=100000;object.scrollLeft=0;cXpix = (result.split("\n").length)*(textareafontsize+3);
cYpix = (result.length-result.lastIndexOf("\n")-1)*(textreafontwidth+1);taXpix = (object.rows+1)*(textareafontsize+3);taYpix = object.clientWidth;
if((cXpix>pXpix)&&(cXpix<(pXpix+taXpix))) object.scrollTop=pXpix;if(cXpix<=pXpix) object.scrollTop=cXpix-(textareafontsize+3);
if(cXpix>=(pXpix+taXpix)) object.scrollTop=cXpix-taXpix;if((cYpix>=pYpix)&&(cYpix<(pYpix+taYpix))) object.scrollLeft=pYpix;
if(cYpix<pYpix) object.scrollLeft=cYpix-(textreafontwidth+1);if(cYpix>=(pYpix+taYpix)) object.scrollLeft=cYpix-taYpix+1;}return true;}else if(IE){
if(isChat){var code = frames['input'].event.keyCode;if(code == 13){return;}
txt=String.fromCharCode(code);cursor_pos_selection = frames['input'].document.selection.createRange();cursor_pos_selection.text="";
cursor_pos_selection.moveStart("character",-1);vorTxt = cursor_pos_selection.text;if(vorTxt.length>1){vorTxt="";}
frames['input'].event.keyCode = 0;if(richtung==2){result = vorTxt+translitBuchstabeLat(txt)}else{result = translitBuchstabeCyr(vorTxt,txt)}
if(vorTxt!=""){cursor_pos_selection.select(); cursor_pos_selection.collapse();}with(frames['input'].document.selection.createRange()){text = result; collapse(); select()}}else{
var code = event.keyCode;if(code == 13){return;}txt=String.fromCharCode(code);cursor_pos_selection = document.selection.createRange();cursor_pos_selection.text="";
cursor_pos_selection.moveStart("character",-1);vorTxt = cursor_pos_selection.text;if(vorTxt.length>1){vorTxt="";}event.keyCode = 0;if (richtung==2){
result = vorTxt+translitBuchstabeLat(txt)}else{result = translitBuchstabeCyr(vorTxt,txt)}
if (vorTxt!=""){cursor_pos_selection.select(); cursor_pos_selection.collapse();}with(document.selection.createRange()){text = result; collapse(); select()}}return;}}
function translateAlltoCyrillic(){if(!IE){txt = TxtFeld.value;var txtnew = translitBuchstabeCyr("",txt.substr(0,1));var symb = "";
for (kk=1;kk<txt.length;kk++){symb = translitBuchstabeCyr(txtnew.substr(txtnew.length-1,1),txt.substr(kk,1));txtnew = txtnew.substr(0,txtnew.length-1) + symb;}
TxtFeld.value = txtnew;setFocus()}else{var is_selection_flag = 1;var userselection = document.selection.createRange();var txt = userselection.text;
if(userselection==null || userselection.text==null || userselection.parentElement==null || userselection.parentElement().type!="textarea"){
is_selection_flag = 0;txt = TxtFeld.value;}var txtnew = translitBuchstabeCyr("",txt.substr(0,1));var symb = "";for (kk=1;kk<txt.length;kk++){
symb = translitBuchstabeCyr(txtnew.substr(txtnew.length-1,1),txt.substr(kk,1));txtnew = txtnew.substr(0,txtnew.length-1) + symb;}
if(is_selection_flag){userselection.text = txtnew; userselection.collapse(); userselection.select();}else{TxtFeld.value = txtnew;setFocus()}}return;}
...
<div class="editorbutton" OnClick="InsertCode('b')"><img class='bold' src='pic/trans.gif' title='Жирный текст'/></div>
<div class="editorbutton" OnClick="InsertCode('i')"><img class='kosoy' src='pic/trans.gif' title='Наклонный текст'/></div>
<div class="editorbutton" OnClick="InsertCode('u')"><img class='podcherknuto' src='pic/trans.gif' title='Подчеркнутый текст'/></div>
<div class="editorbutton" OnClick="InsertCode('s')"><img class='zacherknuto' src='pic/trans.gif' title='Перечеркнутый текст'/></div>
<div class="editorbutton" OnClick="InsertCode('li')"><img class='tochka' src='pic/trans.gif' title='Маркированный список'/></div>
<div class="editorbutton" OnClick="InsertCode('hr')"><img class='liniya' src='pic/trans.gif' title='Разделительная линия'/></div>
<div class="editorbutton" OnClick="InsertCode('left')"><img class='lefttext' src='pic/trans.gif' title='Выравнивание по левому краю'/></div>
<div class="editorbutton" OnClick="InsertCode('center')"><img class='centertext' src='pic/trans.gif' title='Выравнивание по центру'/></div>
<div class="editorbutton" OnClick="InsertCode('right')"><img class='righttext' src='pic/trans.gif' title='Выравнивание по правому краю'/></div>
<div class="editorbutton" OnClick="InsertCode('justify')"><img class='poshirine' src='pic/trans.gif' title='Выравнивание по ширине'/></div>
<div class="editorbutton" OnClick="InsertCode('code')"><img class='codbb' src='pic/trans.gif' title='Код'/></div>
<div class="editorbutton" OnClick="InsertCode('php')"><img class='phpcodebb' src='pic/trans.gif' title='PHP-Код'/></div>
<div class="editorbutton" OnClick="InsertCode('spoiler')"><img class='spoilerbb' src='pic/trans.gif' title='Скрытое содержимое'/></div>
<div class="editorbutton" OnClick="InsertCode('url','Введите полный адрес','Введите описание','Вы не указали адрес!')"><img class='urlbb' src='pic/trans.gif' title='Вставить ссылку'/></div>
<div class="editorbutton" OnClick="InsertCode('mail','Введите полный адрес','Введите описание','Вы не указали адрес!')"><img class='emailbb' src='pic/trans.gif' title='Вставить E-Mail'/></div>
<div class="editorbutton" OnClick="InsertCode('imdb')"><img class='imdbbb' src='pic/trans.gif' title='IMDB'/></div>
<div class="editorbutton" OnClick="InsertCode('kp')"><img class='kpbb' src='pic/trans.gif' title='КиноПоиск'/></div>
<div class="editorbutton" OnClick="InsertCode('img')"><img class='imgbb' src='pic/trans.gif' title='Вставить картинку'/></div>
<div class="editorbutton" OnClick="InsertCode('quote')"><img class='quotebb' src='pic/trans.gif' title='Цитировать'/></div>
<div class="editorbutton" OnClick="InsertCode('youtube')"><img class='youtubebb' src='pic/trans.gif' title='Вставить видеоролик с YouTube. Берем только номер для вставки между кодами'/></div>
<div class="editorbutton" OnClick="InsertCode('rutube')"><img class='rutubebb' src='pic/trans.gif' title='Вставить видеоролик с RuTube. Берем только номер для вставки между кодами'/></div>
<div class="editorbutton" OnClick="translateAlltoCyrillic()"><img class='rusbb' src='pic/trans.gif' title='Перевод текста с латиницы в кириллицу'/></div>
<div class="editorbutton" OnClick="translateAlltoLatin()"><img class='engbb' src='pic/trans.gif' title='Перевод текста с кириллицы в латиницу'/></div>
<div class="editorbutton" OnClick="changelanguage()"><img class='avtoperevod' src='pic/trans.gif' title='Автоматический перевод текста'/></div>
...
<td class='zaliwka' style='font-weight:bold;font-family:Tahoma,Geneva,sans-serif;color:#FFFFFF;font-size:14px;text-align:left;border:0;border-radius:5px;width:200px;'>&nbsp;&nbsp;&nbsp;&nbsp;
...
$start_html = "<div align='left' style='width:85%;overflow:auto;'><table style='float:left;width:avto;background:none;padding-left:5px;margin-top:7px;padding-right:15px;border:0;'>
<tr><td style='background:none;border:0;width:100%;'><table style='background:none;width:100%;float:left;border:0;'><tr>
<td style='background:black;font-weight:bold;font-family:Tahoma,Geneva,sans-serif;color:#FFFFFF;font-size:14px;text-align:left;border:0;border-radius:5px;width:80px;'>&nbsp;&nbsp;&nbsp;&nbsp;
...
$start_html = "<div align='left' style='width:85%;overflow:auto;'><table style='float:left;width:avto;background:none;padding-left:5px;margin-top:7px;padding-right:15px;border:0;'>
<tr><td style='background:none;border:0;width:100%;'><table style='background:none;width:100%;float:left;border:0;'><tr>
<td style='background:darkred;font-weight:bold;font-family:Tahoma,Geneva,sans-serif;color:#FFFFFF;font-size:14px;text-align:left;border:0;border-radius:5px;width:100px;'>&nbsp;&nbsp;&nbsp;&nbsp;
...
////////////////////////////
function format_comment($text, $strip_html = true){global $smilies, $smilies2, $privatesmilies, $DEFAULTBASEURL;
...
global $CURUSER;$cacheus = new Memcache();$cacheus->connect('81.91.178.177', 11211);$cacheused = new Memcache();$cacheused->connect('81.91.178.177', 11211);$count = 0;foreach ($rows as $row){
$rowus = $cacheus->get('user_cache_'.$row['user']);$rowusedit = $cacheused->get('user_cache_'.$row['editedby']);
//////////////////////////
if(strtotime($rowus["last_access"]) > gmtime() - 600){$online = "online";$online_text = "В сети";}else{$online = "offline";$online_text = "Не в сети";}
?><br><table style='background:none;' width='98%' border='0' cellspacing='0' cellpadding='3'><?
if(!empty($rowus["avatar"])){$avatar = $rowus["avatar"];}else{$avatar = "pic/default_avatar.gif";}
if (md5($row['text']) == $row['text_hash'])$text = $row['text_parsed'];else{$text = $row['text_html'];}
...
<a href='user_".$row['user']."'><img src='".$avatar."' width='120' border='1' alt='".($rowus["username"]?($rowus["username"]):"Anonymous")."' title='".($rowus["username"]?($rowus["username"]):"Anonymous")."'/></a><br>");
if(isset($rowus["username"])){
print("<a href='user_".$row["user"]."'><b>".get_user_class_color($rowus["class"], htmlspecialchars_uni($rowus["username"]))."</b></a>
".($rowus["donor"] == "yes" ? "&nbsp;<img src='pic/star.gif' alt='Donor'/>" : "").($rowus["warned"] == "yes" ? "&nbsp;<img src='pic/warned.gif' alt='Warned'/>" : "")."<hr width='120'>
...
<td class='zaliwka' style='font-family:Tahoma,Geneva,sans-serif;color:#FFFFFF;colspan:16;font-size:14px;border:0;border-radius:5px;'>
<div style='font-family:Tahoma,Geneva,sans-serif;font-size:11px;font-weight:10;color:#FFFFFF;margin-left:10px;letter-spacing:0;text-align:left;float:left;border:0;'>
...
print("&nbsp;&nbsp;&nbsp;<span style='color:#FFFFFF;font-family:Tahoma,Geneva,sans-serif;font-size:11px;margin:0;margin-top:3px;padding:2px;letter-spacing:0;'>
Last edited by <a href='user_".$row['editedby']."'><b>".$rowusedit['username']."</b></a> in ".$row['editedat']."</span>");}print("</div>");
...
$queries++;$query_start_time = timer();$result = mysql_query($query);$query_end_time = timer();
...
/////////////////////
function dbconn($autoclean = false, $autocleanusers = false, $lightmode = false){global $mysql_host, $mysql_user, $mysql_pass, $mysql_db;
...
if(basename($_SERVER['SCRIPT_FILENAME']) == 'index.php'){register_shutdown_function("autoclean");register_shutdown_function("autocleanusers");}
...
$client = @$_SERVER['HTTP_CLIENT_IP'];$forward = @$_SERVER['HTTP_X_FORWARDED_FOR'];$remote = @$_SERVER['REMOTE_ADDR'];$agent = $_SERVER["HTTP_USER_AGENT"];
if(filter_var($client, FILTER_VALIDATE_IP)) $ip = $client;elseif(filter_var($forward, FILTER_VALIDATE_IP)) $ip = $forward;
else $ip = $remote;$url = getenv("REQUEST_URI");if($ip != '127.001.001.000' && $agent != 'Transmission/3.00'){ // прописать IP вашего сервера! //прописать ваши данные!//
...
//////////////////
function autocleanusers(){global $autoclean_users, $rootpath;$now = time();$docleanupusers = 0;    
$res = sql_query("SELECT value_u FROM avps WHERE arg = 'lastcleanuserstime'");$row = mysql_fetch_array($res);
if(!$row){sql_query("INSERT INTO avps (arg, value_u) VALUES ('lastcleanuserstime',$now)");return;}$ts = $row[0];if($ts + $autoclean_users > $now) return;
if($ts > $now){sql_query("UPDATE avps SET value_u=$now WHERE arg='lastcleanuserstime' AND value_u = $ts");return;}
sql_query("UPDATE avps SET value_u=$now WHERE arg='lastcleanuserstime' AND value_u = $ts");
if(!mysql_affected_rows()) return;require_once($rootpath.'include/cleanupuser.php');docleanupusers();}
//////////////////////////////
...
header('Content-Type:text/html;charset=utf-8');header('X-Powered-by:LoLi v4.0');header('Cache-Control:public, max-age=86400');if($CURUSER){
...
</td><?global $CacheBlock, $rootpath;$_cache2 = 'freleech.cache';$freleech_cache = $rootpath."include/cache/freleech.cache";if(!file_exists($freleech_cache)){
...
<td style="border-radius:15px;border:none;" width="98%" class="a"><div align="center"><a href="#top" title='ВВерх!' style="position:fixed;bottom:10pt;right:10pt;font-size:26px;'">⮉</a></div>
...
print("<b><a target=_blank title='Форум поддержки и разработки LoLi' class='but' href='https://tbdev-forum.top'>".LoLi."</a><br>Page generated in <b>".$seconds."</b> seconds with ".$queries_staff."");?>
...
///////////////
function deletetorrent($id){global $rootpath;$images = mysql_fetch_array(sql_query('SELECT image1 FROM torrents WHERE id = '.$id));if($images){
$ifilenameims1 = $rootpath."torrents/images/".$images["image1"];if(file_exists($ifilenameims1)){unlink($ifilenameims1);}}
...
sql_query('DELETE FROM browse WHERE id = '.$id);
...
$ret = sql_query("SELECT textt FROM relizi_block ORDER BY id DESC $limit") or sqlerr(__FILE__, __LINE__); 
...
static $showed_show_hide;if(!$showed_ajax && !$noblocks){$GLOBALS ['showed_ajax'] = true;}$orbital_cache = array(); 
...
///////////////////////
$smilies2 = array(":nonono:" => "nonono.gif", ":kawaii:" => "kawaii.gif", ":huh2:" => "huh.gif", ":hm:" => "hmm.gif", ":hehe:" => "hehe.gif", ":happy:" => "happy.gif", 
":haha:" => "haha.gif", ":good:" => "good.gif", ":cry:" => "cry1.gif", ":cool2:" => "cool.gif", ":bah:" => "bah.gif", ":ah:" => "ah.gif", ":sad:" => "sad.gif", 
":cry2:" => "cry2.gif", ":cry3:" => "cry3.gif", ":dunno:" => "dunno.gif", ":pleased:" => "pleased.gif", ":advise:" => "advise.gif", ":agr:" => "agr.gif", ":angry:" => "angry1.gif", 
":angry2:" => "angry2.gif", ":bath:" => "bath.gif", ":bye:" => "bye.gif", ":cry3:" => "cry3.gif", ":depressed:" => "depressed.gif", ":frozen:" => "frozen.gif", 
":greedy:" => "greedy.gif", ":hi2:" => "hi.gif", ":hopeless:" => "hopeless.gif", ":hotness:" => "hotness.gif", ":idontcare:" => "idontcare.gif", ":dead:" => "imdead.gif", 
":khekhe:" => "khekhe.gif", ":knife:" => "knife.gif", ":lol:" => "lol.gif", ":love:" => "love1.gif", ":love2:" => "love2.gif", ":omg:" => "omg.gif", ":penalty:" => "penalty.gif", 
":roar:" => "roar.gif", ":shy:" => "shy.gif", ":slap:" => "slap.gif", ":sleep:" => "sleep.gif", ":smoke:" => "smoke.gif", ":stfu:" => "stfu.gif", ":tired:" => "tired1.gif", 
":tired2:" => "tired2.gif", ":tuktuk:" => "tuktuk.gif", ":uhuh:" => "uhuh.gif", ":wall:" => "wall.gif", ":wat:" => "wat1.gif", ":wat2:" => "wat2.gif", ":wind:" => "wind.gif", 
":yawn1:" => "yawn1.gif", ":yawn2:" => "yawn2.gif",); 
///////////////////
$smilies = array(":-)" => "smile1.gif", ":smile:" => "smile2.gif", ":-D" => "grin.gif", ":lol:" => "laugh.gif", ":w00t:" => "w00t.gif", ":-P" => "tongue.gif", ";-)" => "wink.gif", 
":-|" => "noexpression.gif", ":-/" => "confused.gif", ":-(" => "sad.gif", ":'-(" => "cry.gif", ":weep:" => "weep.gif", ":-O" => "ohmy.gif", ":o)" => "clown.gif", 
"8-)" => "cool1.gif", "|-)" => "sleeping.gif", ":innocent:" => "innocent.gif", ":whistle:" => "whistle.gif", ":unsure:" => "unsure.gif", ":closedeyes:" => "closedeyes.gif", 
":cool:" => "cool2.gif", ":fun:" => "fun.gif", ":thumbsup:" => "thumbsup.gif", ":thumbsdown:" => "thumbsdown.gif", ":blush:" => "blush.gif", ":unsure:" => "unsure.gif", 
":yes:" => "yes.gif", ":no:" => "no.gif", ":love:" => "love.gif", ":?:" => "question.gif", ":!:" => "excl.gif", ":idea:" => "idea.gif", ":arrow:" => "arrow.gif", 
":arrow2:" => "arrow2.gif", ":hmm:" => "hmm.gif", ":hmmm:" => "hmmm.gif", ":bred:" => "bred.gif", ":huh:" => "huh.gif", ":geek:" => "geek.gif", ":look:" => "look.gif", 
":rolleyes:" => "rolleyes.gif", ":kiss:" => "kiss.gif", ":shifty:" => "shifty.gif", ":blink:" => "blink.gif", ":smartass:" => "smartass.gif", ":sick:" => "sick.gif", 
":crazy:" => "crazy.gif", ":wacko:" => "wacko.gif", ":alien:" => "alien.gif", ":wizard:" => "wizard.gif", ":wave:" => "wave.gif", ":wavecry:" => "wavecry.gif", 
":baby:" => "baby.gif", ":angry:" => "angry.gif", ":ras:" => "ras.gif", ":sly:" => "sly.gif", ":devil:" => "devil.gif", ":evil:" => "evil.gif", ":evilmad:" => "evilmad.gif", 
":sneaky:" => "sneaky.gif", ":axe:" => "axe.gif", ":slap:" => "slap.gif", ":wall:" => "wall.gif", ":rant:" => "rant.gif", ":jump:" => "jump.gif", ":yucky:" => "yucky.gif", 
":nugget:" => "nugget.gif", ":smart:" => "smart.gif", ":shutup:" => "shutup.gif", ":shutup2:" => "shutup2.gif", ":crockett:" => "crockett.gif", ":zorro:" => "zorro.gif", 
":snap:" => "snap.gif", ":beer:" => "beer.gif", ":beer2:" => "beer2.gif", ":drunk:" => "drunk.gif", ":strongbench:" => "strongbench.gif", ":weakbench:" => "weakbench.gif", 
":dumbells:" => "dumbells.gif", ":music:" => "music.gif", ":stupid:" => "stupid.gif", ":dots:" => "dots.gif", ":offtopic:" => "offtopic.gif", ":spam:" => "spam.gif", 
":oops:" => "oops.gif", ":lttd:" => "lttd.gif", ":please:" => "please.gif", ":sorry:" => "sorry.gif", ":hi:" => "hi.gif", ":yay:" => "yay.gif", ":cake:" => "cake.gif", 
":hbd:" => "hbd.gif", ":band:" => "band.gif", ":punk:" => "punk.gif", ":rofl:" => "rofl.gif", ":bounce:" => "bounce.gif", ":mbounce:" => "mbounce.gif", 
":thankyou:" => "thankyou.gif", ":gathering:" => "gathering.gif", ":hang:" => "hang.gif", ":chop:" => "chop.gif", ":rip:" => "rip.gif", ":whip:" => "whip.gif", 
":judge:" => "judge.gif", ":chair:" => "chair.gif", ":tease:" => "tease.gif", ":box:" => "box.gif", ":boxing:" => "boxing.gif", ":guns:" => "guns.gif", ":shoot:" => "shoot.gif", 
":shoot2:" => "shoot2.gif", ":flowers:" => "flowers.gif", ":wub:" => "wub.gif", ":lovers:" => "lovers.gif", ":kissing:" => "kissing.gif", ":kissing2:" => "kissing2.gif", 
":console:" => "console.gif", ":group:" => "group.gif", ":hump:" => "hump.gif", ":hooray:" => "hooray.gif", ":happy2:" => "happy2.gif", ":clap:" => "clap.gif", 
":clap2:" => "clap2.gif", ":weirdo:" => "weirdo.gif", ":yawn:" => "yawn.gif", ":bow:" => "bow.gif", ":dawgie:" => "dawgie.gif", ":cylon:" => "cylon.gif", ":book:" => "book.gif", 
":fish:" => "fish.gif", ":mama:" => "mama.gif", ":pepsi:" => "pepsi.gif", ":medieval:" => "medieval.gif", ":rambo:" => "rambo.gif", ":ninja:" => "ninja.gif", 
":hannibal:" => "hannibal.gif", ":party:" => "party.gif", ":snorkle:" => "snorkle.gif", ":evo:" => "evo.gif", ":king:" => "king.gif", ":chef:" => "chef.gif", 
":mario:" => "mario.gif", ":pope:" => "pope.gif", ":fez:" => "fez.gif", ":cap:" => "cap.gif", ":cowboy:" => "cowboy.gif", ":pirate:" => "pirate.gif", ":pirate2:" => "pirate2.gif", 
":rock:" => "rock.gif", ":cigar:" => "cigar.gif", ":icecream:" => "icecream.gif", ":oldtimer:" => "oldtimer.gif", ":trampoline:" => "trampoline.gif", ":banana:" => "bananadance.gif", 
":smurf:" => "smurf.gif", ":yikes:" => "yikes.gif", ":osama:" => "osama.gif", ":saddam:" => "saddam.gif", ":santa:" => "santa.gif", ":indian:" => "indian.gif", ":pimp:" => "pimp.gif", 
":nuke:" => "nuke.gif", ":jacko:" => "jacko.gif", ":ike:" => "ike.gif", ":greedy:" => "greedy.gif", ":super:" => "super.gif", ":wolverine:" => "wolverine.gif", ":spidey:" => "spidey.gif", 
":spider:" => "spider.gif", ":bandana:" => "bandana.gif", ":construction:" => "construction.gif", ":sheep:" => "sheep.gif", ":police:" => "police.gif", ":detective:" => "detective.gif", 
":bike:" => "bike.gif", ":fishing:" => "fishing.gif", ":clover:" => "clover.gif", ":horse:" => "horse.gif", ":shit:" => "shit.gif", ":soldiers:" => "soldiers.gif",);
///////////////////////
$privatesmilies = array(":)" => "smile1.gif", ":wink:" => "wink.gif", ":D" => "grin.gif", ":P" => "tongue.gif", ":(" => "sad.gif", ":'(" => "cry.gif", ":|" => "noexpression.gif", 
":Boozer:" => "alcoholic.gif", ":deadhorse:" => "deadhorse.gif", ":spank:" => "spank.gif", ":yoji:" => "yoji.gif", ":locked:" => "locked.gif", ":grrr:" => "angry.gif", 
"O:-" => "innocent.gif", ":sleeping:" => "sleeping.gif", "-_-" => "unsure.gif", ":clown:" => "clown.gif", ":mml:" => "mml.gif", ":rtf:" => "rtf.gif", ":morepics:" => "morepics.gif", 
":rb:" => "rb.gif", ":rblocked:" => "rblocked.gif", ":maxlocked:" => "maxlocked.gif", ":hslocked:" => "hslocked.gif",);$linebreak = "\r\n";
////////////////////
