<? if (!defined('BLOCK_FILE')){header("Location: ../");exit;}global $CacheBlock, $serverloadBlock_Refresh;
$blocktitle = ".:: Загрузка сервера ::.";$_cache = 'Blocks_serverload.cache';if(!$CacheBlock->Check($_cache, $serverloadBlock_Refresh?0:36000)){ //10 chasov 
...
if (mysql_modified_rows()==0)mysql_query("INSERT INTO avps (arg, value_u, value_i, value_s) VALUES ('load_peers', ".sqlesc($now).", ".sqlesc($peers).",".sqlesc($dttime).")");
...
if(mysql_modified_rows()==0)mysql_query("INSERT INTO avps (arg, value_u, value_i, value_s) VALUES ('load_connected', ".sqlesc($now).", ".sqlesc($connected).",".sqlesc($dttime).")");
...
if(!empty($percent)){
$content.= "<td class='a' align='center' colspan='2'><table border='0' width='402px'><tr><td style='padding:0px;background-repeat:repeat-x;'>
...
$content.= "<td class='a' align='center' colspan='2'><table border='0' width='400px'><tr>
