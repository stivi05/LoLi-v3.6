<?if(!defined('BLOCK_FILE')){header("Location: ../index.php");exit;}$cacheStatFile = "include/cache/block-server_load.txt";$expire = 60*60; // 1 chas
if (file_exists($cacheStatFile) && filesize($cacheStatFile)<>0 && filemtime($cacheStatFile) > (time() - $expire)){ 
$content.=file_get_contents($cacheStatFile);}else{$blocktitle = $tracker_lang['server_load'];
...
elseif(!empty($arra["load_peers"]["value_i"])){$toppeer = "Max: <b>".$arra["load_peers"]["value_i"]."</b> в ".get_date_time($arra["load_peers"]["value_u"])."";}
...
$connected_guest = mysql_num_rows(sql_query("SELECT userid FROM peers WHERE userid='0'"));if($arra["load_guest"]["value_i"] < $connected_guest){
...
if(strtolower(substr(PHP_OS, 0, 3)) != 'win')$percent = $avgload;else $percent = $avgload;
...
<img height=\"15\" width=\"".$width."\" src=\"pic/".$pic."\" title=\"Нагрузка: ".$percent."%, Средняя (LA): ".$avgload."\"></td></tr></table><tr>
<td class=\"b\" align=\"center\" colspan=\"2\"><b>Нагрузка</b>: ".$percent."%, <b>Средняя</b> (LA): ".$avgload."</tr></td></td>";}
if(!empty($peers)){$fix_bug = ($arra["load_peers"]["value_i"]/100);if($fix_bug > 1){$fix_peers = ($peers*100);}else{$fix_peers = $peers;}
$fix_peers_value = ($arra["load_peers"]["value_i"]);$fix_peers = $fix_peers_value;if ($fix_peers <= 10)$pic_ = "loadbarbg.gif";
elseif($fix_peers <= 50)$pic_ = "loadbargreen.gif";elseif($fix_peers <= 70)$pic_ = "loadbaryellow.gif";else $pic_ = "loadbarred.gif";$width_ = $fix_peers * 4;	
...
if(!empty($session_now)){$fix_bug = ($session_all/100);
if($fix_bug > 1){$fix_sessions = ($session_now*100);}else{$fix_sessions = $session_now;}$fix_sessions = $fix_sessions/$session_all;
if($fix_sessions <= 10)$pic_2 = "loadbarbg.gif";elseif($fix_sessions <= 50)$pic_2 = "loadbargreen.gif";
...
$content.= "</tr></table>";$fp = fopen($cacheStatFile,"w");if($fp){fputs($fp, $content);fclose($fp);}}?>