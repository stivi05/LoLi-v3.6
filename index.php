<?php require_once("include/bittorrent.php");dbconn(true);gzip();if($CURUSER){if(get_user_class() > UC_VLADELEC || get_user_class() < UC_USER){
...
global $CacheBlock, $tracker_lang, $SITENAME;$ip = getip();$_cache = 'signup.cache';$_cache2 = 'inwayts.cache';$signup = $CacheBlock->Read($_cache);
$inwayts = $CacheBlock->Read($_cache2);$banes = sql_query("SELECT first, haker FROM bans WHERE first = '$ip'");$bancount = mysql_num_rows($banes);if($bancount){
...
</body></html><? die;}else{$newres = sql_query("SELECT username FROM users WHERE ip = '$ip'");$newcount = mysql_num_rows($newres);
if($newcount){$signup1 = "";$signup2 = "";}else{$signup1 = "<span id='signup'>Sign Up</span>";$signup2 = "<span id='signup2'>Sign Up</span>";}
...
</head><body><div id="login-button" style='font-size:96px;' title="login">👤</div>
