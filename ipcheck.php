...
$res = sql_query("SELECT ip, COUNT(ip) AS count FROM users GROUP BY ip HAVING COUNT(ip) > 1") or sqlerr(__FILE__, __LINE__);
$row2 = mysql_fetch_array($res);$count = $row2['count'];$ip = $row2['ip'];if($count <=1){?>
<center><font style='font-family:tahoma;font-size:18px;font-weight:10;color:red;'><b>Нет Дублей! Или юзеры используют VPN для смены IP.</b></font></center><?}else{
$ress = sql_query("SELECT id, username, class, email, added, last_access, downloaded, uploaded, warned, donor, enabled, (seeder + leecher) AS peer_count FROM users WHERE ip = '$ip' ORDER BY ip DESC") or sqlerr(__FILE__, __LINE__);
...
if($arr['last_access'] == '0000-00-00 00:00:00')$arr['last_access'] = '-';
...
<td align='center'>$ratio</td><td align='center'><span style='font-weight:bold;'>$ip</span></td><td align='center'>
