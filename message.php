...
if(mysql_num_rows($res) == 0){stderr2($tracker_lang['error'],"Такого сообщения не существует.");}$message = mysql_fetch_assoc($res);
...
(".nicetime($message["added"], true).")</div><div style='font-family:tahoma;color:#FFFFFF;margin-left:10px;letter-spacing:0;text-align:left;float:left;border:0;'>
...
$auto = $_GET["auto"];$std = $_GET["std"];if((!$auto || !$std) && get_user_class() < UC_MODERATOR)stderr2($tracker_lang['error'], "Досступ запрещен.");
...
///////////////////////////////
stdhead("Отправка сообщения", false);?><table style="background:none;cellspacing:0;cellpadding:0;width:100%;float:center;"><tr>
...
if(!$_cacheus) stderr("<center>Error!</center>", "<center>Нет пользователя с таким ID $id.</center><html><head><meta http-equiv=refresh content='5;url=/'></head></html>");
...
/////////////////////////
$receiver_class = $user['class'];$receiver_avatar = $user['avatar'];$receiver_username = $user['username'];
sql_query("INSERT INTO messages (sender, sender_class, sender_username, sender_avatar, receiver, receiver_class, receiver_username, receiver_avatar, added, msg, subject, 
...
if(mysql_num_rows($res) == 0){stderr2($tracker_lang['error'],"Сообщения с таким ID не существует.");}
$message = mysql_fetch_assoc($res);
if($message['receiver'] == $CURUSER['id'] && $message['saved'] == 'no'){$res2 = sql_query("DELETE FROM messages WHERE id=".sqlesc($pm_id)) or sqlerr(__FILE__,__LINE__);}
elseif($message['sender'] == $CURUSER['id'] && $message['location'] == PM_DELETED){$res2 = sql_query("DELETE FROM messages WHERE id=".sqlesc($pm_id)) or sqlerr(__FILE__,__LINE__);}
elseif($message['receiver'] == $CURUSER['id'] && $message['saved'] == 'yes'){$res2 = sql_query("UPDATE messages SET location=0 WHERE id=".sqlesc($pm_id)) or sqlerr(__FILE__,__LINE__);}
elseif($message['sender'] == $CURUSER['id'] && $message['location'] != PM_DELETED){$res2 = sql_query("UPDATE messages SET saved='no' WHERE id=".sqlesc($pm_id)) or sqlerr(__FILE__,__LINE__);}
if(!$res2){stderr2($tracker_lang['error'],"Невозможно удалить сообщение.");}
if(mysql_affected_rows() == 0){stderr2($tracker_lang['error'],"Невозможно удалить сообщение.");}else{header("Location: mess_in");exit();}}
