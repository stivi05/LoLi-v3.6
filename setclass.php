<?php require_once("include/bittorrent.php");dbconn(true);gzip();if($CURUSER && get_user_class() >= UC_MODERATOR && $CURUSER['override_class'] == 255){
...
$cache = new Memcache();$cache->connect('81.91.178.177', 11211); //прописать IP вашего сервера!//
$cache->delete('user_cache_'.$CURUSER['id']);header("Refresh: 0; url=/");}stdhead("Смена класса");begin_frame(".:: Cмена класса ::.");?>
<form method='get' action='setclass.php'><table width='100'%' border='0' cellspacing='5' cellpadding='5'><center><input type='hidden' name='action' value='editclass'>
