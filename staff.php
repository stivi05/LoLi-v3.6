...
$res = sql_query("SELECT users.id, users.avatar, users.country, users.gender, users.username, users.class, users.last_access, c.name, c.flagpic FROM users LEFT JOIN countries AS c ON c.id = users.country WHERE users.class>=".UC_UPLOADER." AND users.status='confirmed' ORDER BY users.username" ) or sqlerr(__FILE__, __LINE__);
while($arr = mysql_fetch_assoc($res)){
if($arr['country'] > 0){$country = "<img src='pic/flag/".$arr['flagpic']."' alt='".$arr['name']."' title='".$arr['name']."'/>";}else{$country = "<Не указанно";}
if(!$arr['avatar']){$avatar=("<a href='userdetails.php?id=".$arr['id']."'><img height='50px' border='0' title='".$arr['username']."' src='pic/default_avatar.gif'/></a>");}
else{$avatar=("<a href='userdetails.php?id=".$arr['id']."'><img height='50px' src='".$arr['avatar']."' title='".$arr['username']."'/></a>");}
...
<a href='#' onclick=\"javascript:window.open('sendpm.php?action=sendmessage&receiver=".$arr['id']."', 'Отправить PM', 'width=650px, height=465px');return false;\" title='Отправить ЛС'><img src='pic/pn_inbox.gif' border='0' title='PM'/></a>&nbsp;&nbsp;$country</center></td></tr></table><hr width='200px'><table style='background:none;border:0;' width='240'><center>
...
</td></tr></table><table style='background:none;margin-top:3px;cellspacing:0;cellpadding:0;width:100%;float:center;'><tr height='15'>".$staff_table[UC_SYSOP]."</tr>";
if($staff_table[UC_ADMINISTRATOR]){$content.= "<tr height='15'>".$staff_table[UC_ADMINISTRATOR]."</tr>";}
if($staff_table[UC_MODERATOR]){$content.= "<tr height='15'>".$staff_table[UC_MODERATOR]."</tr>";}
...
</table><table style='background:none;margin-top:3px;cellspacing:0;cellpadding:0;width:100%;float:center;'><tr height='15'>".$staff_table[UC_UPLOADER]."</tr>";
