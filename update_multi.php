<?php require_once("include/bittorrent.php");dbconn();$old_us = $use_sessions;$use_sessions = 0;$use_sessions = $old_us;require_once('include/httptscraper.php');
...
if ($name == '' || $multitracker == 'no'){stderr2($tracker_lang['error'], "<center>Такого торрента нет, или он не мультитрекерный.</center><html><head><meta http-equiv=refresh content='1;url=/'></head></html>");}
if(strtotime($last_mt_update) > (TIMENOW - 3600)){
stderr($tracker_lang['error'], "<center>Вы пытаетесь обновить мультитрекер слишком часто. Разрешено это делать не чаще 1 раза в час.</center><html><head><meta http-equiv=refresh content='1;url=details.php?id=".$tid."'></head></html>");}
...
mysql_query('UPDATE torrents AS t INNER JOIN (SELECT ts.tid, SUM(ts.seeders) AS sum_seeders, SUM(ts.leechers) AS sum_leechers FROM torrents_scrape AS ts 
WHERE ts.tid = '.$tid.' GROUP BY ts.tid) AS ts ON ts.tid = t.id SET t.remote_seeders = ts.sum_seeders, t.remote_leechers = ts.sum_leechers, t.last_action = NOW(), 
t.last_mt_update = NOW(), visible = IF(t.remote_seeders > 0, "yes", visible) WHERE t.id = '.$tid) or sqlerr(__FILE__,__LINE__);
$ajax = strval($_GET['ajax']);if($ajax == 'yes'){$errors = count($works) - $success;
stderr($tracker_lang['success'], "<center>Обновление мультитрекера выполнено успешно. Успешно: $success Ошибок: $errors</center><html><head><meta http-equiv=refresh content='1;url=details.php?id=".$tid."'></head></html>");
...
stderr($tracker_lang['success'], "<center><ul style='margin:0;'>".implode($anns)."</ul>".$update_link."</center><html><head><meta http-equiv=refresh content='1;url=details.php?id=".$tid."'></head></html>");
}else{mysql_query("UPDATE torrents SET multitracker = 'no' WHERE id = $tid");
stderr($tracker_lang['error'], "<center>WTF? Multitracker = YES, but no announces</center><html><head><meta http-equiv=refresh content='1;url=details.php?id=".$tid."'></head></html>");}}?>
