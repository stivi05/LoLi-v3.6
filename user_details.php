...
if($user["ip"] && (get_user_class() >= UC_MODERATOR || $user["id"] == $CURUSER["id"])){$ip = $user["ip"];$addr = "<br><br>&nbsp;&nbsp;<b>IP:</b>&nbsp;$ip";}   
////////////////////////////////////////////////////
//////////// скачал начало /////////////////////
...
&nbsp;&nbsp;<b>Раздал(a):</b> <?=mksize($user["uploaded"])?>&nbsp;(<?=perday($user['added'],$user["uploaded"])?>&nbsp;<?=$tracker_lang['perday']?>)<br><br>
&nbsp;&nbsp;<b>Скачал(a):</b> <?=mksize($user["downloaded"])?>&nbsp;(<?=perday($user['added'],$user["downloaded"])?>&nbsp;<?=$tracker_lang['perday']?>)<br><br>
&nbsp;&nbsp;<b>Всего трафика:</b> <?=mksize($user["downloaded"] + $user["uploaded"])?><br><br>
...
&nbsp;&nbsp;<b>Рейтинг:</b> <?=$sr?><br><br>
&nbsp;&nbsp;<b>Бонус:</b> <?=$user["bonus"]?><br><br>
&nbsp;&nbsp;<b>Комментариев Релизов:</b>
...
<br><br>&nbsp;&nbsp;<b>Поблагодарили юзера:</b> <?=$user['simpaty'];?> раз
<?if ($user["website"]){?><br><br>&nbsp;&nbsp;<b>Сайт:</b>&nbsp;&nbsp;<a href="<?=$user['website']?>" target="_blank"><b><?=$user['website']?></b></a><?}?>
<?if($addr)?><?=$addr?><?$dt = gmtime() - 300;$dt = sqlesc(get_date_time($dt));
...
</td><td valign="top" style="border:0;" align="left">&nbsp;&nbsp;<?=$country?><br><br>
&nbsp;&nbsp;<b>Зарегистрирован:</b> <?=$joindate?><br><br>
&nbsp;&nbsp;<b>Последнее посещение:</b> <?=$lastseen?><br><br>
&nbsp;&nbsp;<b>Аккаунт припаркован:</b> <?if($user['parked'] == 'no'){?>Нет<?}else{?>Да<?}?><br><br>
...
&nbsp;&nbsp;<b>Пригласил:</b> <b><a href="userdetails.php?id=<?=$user['invitedby']?>"><?=$inviter['username']?></a></b><br><br><?}}?>
<?$birthday = date("d.m.Y", strtotime($birthday));?>
&nbsp;&nbsp;<b>Дата Рождения:</b> <?=$birthday?><br><br>
&nbsp;&nbsp;<b>Возраст:</b> <?=$age?> лет</a><br><br>
<?if($user["telgr"] || $user["skype"]){?>&nbsp;&nbsp;<b>Связь:</b><?if($user["telgr"]) print("&nbsp;&nbsp;<a href='https://t.me/$user[telgr]' target='_blank'><img src=\"pic/tlgr.png\" alt=\"Telegram\" border=\"0\" title=\"Telegram\" />&nbsp;$user[telgr]</a>");if ($user["skype"]) print("&nbsp;&nbsp;<img src=\"pic/contact/skype.gif\" alt=\"skype\" border=\"0\" title=\"skype\" /> $user[skype]");?></b><br><br><?}?>
<?if(get_user_class() >= UC_MODERATOR || $user["id"] == $CURUSER["id"]){?>&nbsp;&nbsp;<b>Email:</b> <b><a href="mailto:$user[email]"><?=$user['email']?></a></b><br><br><?}?>
<?if (get_user_class() >= UC_MODERATOR || $user["id"] == $CURUSER["id"]){?>&nbsp;&nbsp;<b>Приглашений:</b> <a href="invite.php?id=<?=$id?>"><?=$user["invites"]?></a><br><br><?}?>
&nbsp;&nbsp;<a href="donate.php"><b>Donations:</b></a> &euro;<b><?=$user["donated"]?></b><br><br><?
...
&nbsp;&nbsp;<b>Email Юзера:</b>&nbsp;&nbsp;<input type="text" size="40" name="emails" value="<?=htmlspecialchars_uni($user[email])?>"><br><br>
&nbsp;&nbsp;<input type="text" size="40" name="title" value="<?=htmlspecialchars_uni($user[title])?>"><br><br>
&nbsp;&nbsp;<b>Аватар:</b>&nbsp;&nbsp;
...
<? if(get_user_class() == UC_MODERATOR){
print("&nbsp;&nbsp;<b>Класс:</b>&nbsp;&nbsp;<input type=\"hidden\" name=\"class\" value=\"$user[class]\"><br><br>");
}else{print("&nbsp;&nbsp;<b>Класс:</b>&nbsp;&nbsp;<select name=\"class\">");
$maxclass = get_user_class();
for ($i = 0; $i <= $maxclass; ++$i) print("<option value=\"$i\"".($user["class"] == $i ? " selected" : "").">$prefix".get_user_class_name($i)."");
print("</select><br><br>");}
if($CURUSER["class"] < UC_ADMINISTRATOR){?>&nbsp;&nbsp;<b>Донор:</b>&nbsp;&nbsp;<input type="hidden" name="donor" value="<?=$user[donor]?>">
<br><br><?}else{?>&nbsp;&nbsp;<b>Тема оформления:</b>&nbsp;&nbsp;<?=$themes = theme_selector($user["theme"]);?><br><br><?
print("&nbsp;&nbsp;<b>Донор:</b>&nbsp;&nbsp;<input type=\"radio\" name=\"donor\" value=\"yes\"".($user["donor"] == "yes" ? " checked" : "").">Да <input type=\"radio\" name=\"donor\" value=\"no\"".($user["donor"] == "no" ? " checked" : "").">Нет<br><br>");}	
...
print("&nbsp;&nbsp;<b>Поддержка:</b>&nbsp;&nbsp;<input type=radio name=support value=yes".($user["support"] == "yes" ? " checked" : "").">Да <input type=radio name=support value=no".($user["support"] == "no" ? " checked" : "").">Нет<br><br>");
print("&nbsp;&nbsp;<b>Поддержка для:</b>&nbsp;&nbsp;<textarea cols=60 rows=3 name=supportfor>$supportfor</textarea><br><br>");
print("&nbsp;&nbsp;<b>История пользователя:</b>&nbsp;&nbsp;<textarea cols=60 rows=6".(get_user_class() < UC_SYSOP ? " readonly" : " name=modcomment").">$modcomment</textarea><br><br>");
print("&nbsp;&nbsp;<b>Добавить заметку:</b>&nbsp;&nbsp;<textarea cols=60 rows=3 name=modcomm></textarea><br><br>");
...
print("&nbsp;&nbsp;<select name=country>$countries</select><br><br>");
print("&nbsp;&nbsp;<b>Сбросить день рождения:</b>&nbsp;&nbsp;
...
//////////////////////////////////////////
print("&nbsp;&nbsp;<b>Изменить раздачу:</b>&nbsp;&nbsp;<img src=\"pic/plus.gif\" id=\"uppic\" onClick=\"togglepic('$DEFAULTBASEURL','uppic','upchange')\" style=\"cursor: pointer;\">&nbsp;<input type=\"text\" name=\"amountup\" size=\"10\" />&nbsp;<select name=\"formatup\"><option value=\"mb\">MB</option><option value=\"gb\">GB</option></select><br><br>");
print("&nbsp;&nbsp;<b>Изменить скачку:</b>&nbsp;&nbsp;<img src=\"pic/plus.gif\" id=\"downpic\" onClick=\"togglepic('$DEFAULTBASEURL','downpic','downchange')\" style=\"cursor: pointer;\">&nbsp;<input type=\"text\" name=\"amountdown\" size=\"10\" />&nbsp;<select name=\"formatdown\"><option value=\"mb\">MB</option><option value=\"gb\">GB</option></select><br><br>");
print("&nbsp;&nbsp;<b>Снять Бонусы:</b>&nbsp;&nbsp;<img src=\"pic/minus.gif\">&nbsp;<input type=\"text\" name=\"bonup\" size=\"10\" /><br><br>");
print("&nbsp;&nbsp;<b>Добавить Бонусы:</b>&nbsp;&nbsp;<img src=\"pic/plus.gif\">&nbsp;<input type=\"text\" name=\"bondown\" size=\"10\" /><br><br>");
print("&nbsp;&nbsp;<b>Снять Приглашения:</b>&nbsp;&nbsp;<img src=\"pic/minus.gif\">&nbsp;<input type=\"text\" name=\"invup\" size=\"10\" /><br><br>");
print("&nbsp;&nbsp;<b>Добавить Приглашения:</b>&nbsp;&nbsp;<img src=\"pic/plus.gif\">&nbsp;<input type=\"text\" name=\"invdown\" size=\"10\" /><br><br>");
print("&nbsp;&nbsp;<b>Раздает сейчас торрентов:</b>&nbsp;&nbsp;<input type=\"text\" size=\"10\" name=\"seeder\" value=".htmlspecialchars_uni($user['seeder'])."><br><br>");
print("&nbsp;&nbsp;<b>Качает сейчас торрентов:</b>&nbsp;&nbsp;<input type=\"text\" size=\"10\" name=\"leecher\" value=".htmlspecialchars_uni($user['leecher'])."><br><br>");
print("&nbsp;&nbsp;<b>Сбросить passkey:</b>&nbsp;&nbsp;<input name=\"resetkey\" value=\"1\" type=\"checkbox\"><br><br>");
print("&nbsp;&nbsp;<b>Использовать Чат:</b>&nbsp;&nbsp;<input type=radio name=schoutboxpos value=yes" .($user["schoutboxpos"]=="yes" ? " checked" : "") . ">Да <input type=radio name=schoutboxpos value=no" .($user["schoutboxpos"]=="no" ? " checked" : "") . ">Нет<br><br>");
print("&nbsp;&nbsp;<b>Комментарии разрешены:</b>&nbsp;&nbsp;<input type=radio name=comentoff value=yes" .($user["comentoff"]=="yes" ? " checked" : "") . ">Да <input type=radio name=comentoff value=no" .($user["comentoff"]=="no" ? " checked" : "") . ">Нет<br><br>");
$warned = $user["warned"] == "yes";
print("&nbsp;&nbsp;".($warned? "<input name=\"warned\" value=\"yes\" type=\"radio\" checked>Да<input name=\"warned\" value=\"no\" type=\"radio\">Нет" : "<b>Нет</b>")."&nbsp;&nbsp;");
...
print("&nbsp;&nbsp;".($enabled ? "<font color=\"green\"><b>Пользователь включен</b></font>" : "<font color=\"red\"><b>Пользователь отключен</b></font>")."&nbsp;&nbsp;");
$disabler = <<<DIS
...
}else{print("&nbsp;&nbsp;<b>Удалить юзера</b>&nbsp;&nbsp;<input type=\"checkbox\" name=\"deluser\"><br><br>");}?></td></tr><?
print("<tr><td style='border:0;' colspan='3' align='center'><input type=\"submit\" class=\"btn\" value=\"ОК\"></td></tr></table>
