...
<span style='color:#999966;'><b>Заказал:</b></span> <b><a href='userdetails.php?id=".$row['user_id']."'>".get_user_class_color($row['class'], $row['user'])."</a></b>
&nbsp;<a href='#' onclick=\"javascript:window.open('sendpm.php?action=sendmessage&receiver=".$row['user_id']."', 'Отправить PM', 'width=650px, height=465px');return false;\" 
...
print("<hr align='left' width='300px'><span style='color:#999966;'><b>Принял:</b></span> <b><a href='userdetails.php?id=".$row["cl_user_id"]."'>".get_user_class_color($row["cl_user_class"], $row["cl_user"])."</a></b>&nbsp;<a href='#' onclick=\"javascript:window.open('sendpm.php?action=sendmessage&receiver=".$row['cl_user_id']."', 'Отправить PM', 'width=650px, height=465px');return false;\" 
title='Отправить ЛС'><img src='pic/pn_inbox.gif' border='0' title='PM'/></a>");}
...
if($CURUSER["id"] == $row['user_id'] and $row['status'] == "3" or get_user_class() >= UC_MODERATOR and $row ['status'] == "3"){$name = $row['name'];
mysql_query("UPDATE zakaz set status='4' WHERE id = '$id'") or sqlerr(__FILE__, __LINE__);
$ros = mysql_query("SELECT bonus FROM users WHERE id=".$row['user_id']) or sqlerr(__FILE__, __LINE__);$orrr = mysql_fetch_array($ros);$bonus_upp = $orrr['bonus'] - $row['bonus'];
mysql_query("UPDATE users set bonus='$bonus_upp' WHERE id=".$row['user_id']) or sqlerr(__FILE__, __LINE__);
$ros = mysql_query("SELECT bonus FROM users WHERE id=".$row['cl_user_id']) or sqlerr(__FILE__, __LINE__);$orr = mysql_fetch_array($ros);$bonus_up = $orr['bonus'] + $row['bonus'];
...
stderr("Заказ <b>$name</b>", "<center>Вы подтвердили договор!</center><script>setTimeout('document.location.href=\"zakaz.php\"', 3000);</script>" );
}}}elseif($_REQUEST['action'] == "url_add" and $id){$url_r = $_REQUEST['url'];if($url_r == ""){
stderr("Ошибка", "<center>Вы должны заполнить все поля правильно!</center><script>setTimeout('document.location.href=\"zakaz.php\"', 3000);</script>");}else{
...
stderr("Успешно", "<center>Операция прошла успешно!</center><script>setTimeout('document.location.href=\"zakaz.php\"', 3000);</script>" );
...
stderr("Анулировать заказ <b>$name</b>", "<center>Вы анулировали заказ!</center><script>setTimeout('document.location.href=\"zakaz.php\"', 3000);</script>" );
...
stderr("Принят заказ <b>$name</b>", "<center>Вы приняли заказ, теперь вы обязанны выполнить условия или анулируйте договор!</center><script>setTimeout('document.location.href=\"zakaz.php\"', 3000);</script>");
...
stderr("Внимание", "<center>Заказ под номером <b>$name</b> успешно удален!</center><script>setTimeout('document.location.href=\"zakaz.php\"', 3000);</script>");
}}}elseif($_REQUEST['action'] == "doadd" or $_REQUEST['action'] == "doedit"){
if($_REQUEST['action'] == "doadd"){
$name = $_REQUEST['name'];$or_name = $_REQUEST['or_name'];$yearsd = (isset($_REQUEST['god']) ? intval($_REQUEST['god']):0);
$years = unesc($yearsd);if (!$years)bark("Вы должны ввести Год выхода!");$god = htmlspecialchars_uni(strval($_REQUEST['god']));$text = unesc($_REQUEST['text']);
...
stderr("Ошибка", "<center>Вы должны заполнить все поля правильно.</center><script>setTimeout('document.location.href=\"zakaz.php\"', 3000);</script>");
...
$uploaddir = "torrents/zakaz/";$ifile = $_FILES['image'.$x]['tmp_name'];$ret = mysql_query("SHOW TABLE STATUS LIKE 'zakaz'");$row = mysql_fetch_array($ret);
$next_id = $row['Auto_increment'];$files = explode('.', $_FILES['image'.$x]['name']);$ifilename = $next_id.'.'.end($files);
$copy = copy($ifile, $uploaddir.$ifilename);if(!$copy)bark("Error occured uploading image! - Image $y");$inames[] = $ifilename;}}
//////////////////////////////
...
stderr("Создание нового заказа", "<center>Новый заказ создан!</center><script>setTimeout('document.location.href=\"zakaz.php\"', 3000);</script>");
}else{
////////////////////
...
for ($x=1; $x <= 5; $x++){$_GLOBALS['img'.$x.'action'] = $_POST['img'.$x.'action'];if($_GLOBALS['img'.$x.'action'] == 'keep'){$image1 = sqlesc($row["image1"]);}
...
stderr("Редактирование заказа", "<center>Редактирование заказа прошло успешно!</center><script>setTimeout('document.location.href=\"zakaz.php\"', 2000);</script>");
}}}elseif($_REQUEST['action'] == "add" or $_REQUEST['action'] == "edit"){
if($_REQUEST['action'] == "add"){$form_action = "?action=doadd";$value = "Подтвердить добавление заказа";$begin_f = "Добавить заказ релиза";}
...
<tr><td align="right" style="border:0;"><b>Опишите подробней Ваш заказ</b></td><td style="border:0;"><?textbbcode("zakaz","text",htmlspecialchars_uni($text));?></td></tr>
<?if($_REQUEST['action'] == "edit"){?>
...
stderr("Внимание", "<center>В данный момент вы не можете совершить это действие!</center><script>setTimeout('document.location.href=\"zakaz.php\"', 1000);</script>");}}else{
...
list($pagertop, $pagerbottom, $limit) = pager($perpage, $count, "zakaz.php?".$q);
...
print("</font>'</td></tr>");}?></form></table></td></tr></table></td></tr></table><?if($count > 10){?>
...
</tr></table></td></tr><tr><td align='left' style='background:none;width:100%;float:center;border:0;'></td></tr></table></td></tr></table><?if($count == 0){?>
<table style='background:none;margin-top:7px;' width='350' border='0'><td style='border-radius:10px;' class='b'><center>
<font style='font-family:tahoma;font-size:18px;font-weight:10;color:red;'><b>Заказов нет.</b></font></center></td></table><?}else{?>
...
<hr width='300px' align='left'><b>Заказал:&nbsp;<a href='userdetails.php?id=".$arr["user_id"]."'>".get_user_class_color($arr["class"], $arr["user"])."</a></b>
&nbsp;<a href='#' onclick=\"javascript:window.open('sendpm.php?action=sendmessage&receiver=".$arr['user_id']."', 'Отправить PM', 'width=650px, height=465px');return false;\" 
title='Отправить ЛС'><img src='pic/pn_inbox.gif' border='0' title='PM'/></a>
...
<tr><td align='center' style='background:none;cellspacing:0;cellpadding:0;width:100%;float:center;border:0;'><?=$pagerbottom?></td></tr></table><?}
...
else if(isset($user)){print("<table style='background:none;margin-top:7px;cellspacing:0;cellpadding:0;width:300px;float:center;'><tr>
<td style='border-radius:15px;border:none;' class='a'><table style='background:none;width:100%;float:center;border:0;'><tr>
<td class='zaliwka' style='color:#FFFFFF;colspan:14;height:40px;font-family:cursive;font-weight:bold;font-size:14px;text-align:center;border:0;border-radius:5px;'>
".$tracker_lang['nothing_found']."</td></tr><tr><td align='center' style='background:none;width:100%;float:center;border:0;'></td></tr></table></td></tr></table>");}
else if(isset($status)){print("<table style='background:none;margin-top:7px;cellspacing:0;cellpadding:0;width:300px;float:center;'><tr>
<td style='border-radius:15px;border:none;' class='a'><table style='background:none;width:100%;float:center;border:0;'><tr>
<td class='zaliwka' style='color:#FFFFFF;colspan:14;height:40px;font-family:cursive;font-weight:bold;font-size:14px;text-align:center;border:0;border-radius:5px;'>
".$tracker_lang['nothing_found']."</td></tr><tr><td align='center' style='background:none;width:100%;float:center;border:0;'></td></tr></table></td></tr></table>");}?>
